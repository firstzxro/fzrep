...
$(function(){
	...
	$.ajax({
		...
		success:function(data){
			...
			// 각 회의실 번호에 따라 함수 호출
	        for (let mtgrNo in prefixes) {
	            processTimeSlot(prefixes[mtgrNo], data.filter(d => d.mtgrNo == mtgrNo), today2);
	        }
		} //listAjax success 끝
	}); //listAjax 끝   
...
function processTimeSlot(prefix, data, today2) {
    for (let i = 0; i < data.length; i++) {
    	
        let mmlNo = data[i].mmlNo;
        let mmlRsvtYNValue = data[i].mmlRsvtYN;
        let mtgrNoValue = data[i].mtgrNo;
        let mmlRsvtStrdtValue = data[i].mmlRsvtStrdt.split(' ')[1].split(':')[0];
        let mmlRsvtEnddtValue = data[i].mmlRsvtEnddt.split(' ')[1].split(':')[0];
        let mmlRsvtDateValue = data[i].mmlRsvtStrdt.split(' ')[0];
        let mmlEmpNo = data[i].empNo;

        for (let hour = 9; hour <= 18; hour++) {
            let timeSlot = document.getElementById(prefix + '-' + hour + '_00');
        ...
        } // for (let hour = 9; hour <= 18; hour++) { 끝
        
    } // for (let i = 0; i < data.length; i++) { 끝
} //function processTimeSlot(prefix, data, today2) 끝
